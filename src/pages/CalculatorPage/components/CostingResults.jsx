...
export function CostingResults({ results, inputs, subTab, onSubTabChange, onReset, formatCurrency }) {
  return (
    <section className="calc-results-section">
      ...
      <div className="calc-cost-banner">
        ...
        <div className="calc-estimate-meta">
          <span>Rate: <strong>₹{Math.round(results.buildingCost.costPerSqft).toLocaleString("en-IN")}/sq.ft</strong></span>
          <span>Area: <strong>{results.buildingCost.totalArea.toLocaleString("en-IN")} sq.ft</strong></span>
          <span>Floors: <strong>{inputs.floors}</strong></span>
          <span>Duration: <strong>{results.timeline.totalDays} days (~{results.timeline.totalMonths} months)</strong></span>
        </div>
      </div>

      {/* Sub Tabs */}
      <div className="calc-tabs">
        {SUB_TABS.filter((t) => !t.conditional || results.stairDesign).map((t) => (
          <button
            key={t.key}
      ...
      <div className="calc-tab-content">
        ...
        {subTab === "structure" && <StructureDesign design={results.structureDesign} />}
        {subTab === "stair"     && results.stairDesign && <StaircaseDesign design={results.stairDesign} />}
        {subTab === "footing"   && <FoundationDesign footing={results.footing} />}
        {subTab === "bbs"       && <CompleteBBS barBending={results.barBending} completeBBS={results.completeBBS} />}
        {subTab === "boq"       && (
          <FullBOQ
      ...
      <div style={{ textAlign: "center", paddingTop: "2rem" }}>
        <button onClick={onReset} className="calc-btn-secondary">← New Estimate</button>
      </div>
    </section>
  );
}