...
function getEscalation(RATES, floorIndex) {
  const keys = Object.keys(RATES.FLOOR_ESCALATION).map(Number).sort((a, b) => a - b);
...
function buildFoundation(plotArea, RATES) {
  const items = [];
  const matQty = { cement: 0, steel: 0, sand: 0, aggregate: 0, bricks: 0, tiles: 0, paint: 0 };
  ...
  const footKey = RATES.GRADE_KEY === 'premium' ? 'rccM25Footing' : 'rccM20Footing';
...
function buildFloorItems(floorArea, plotArea, RATES, floorIndex, isTopFloor) {
  ...
  addItem(isPremium ? 'dadoLargeFormat' : 'dadoCeramic', toSqm(floorArea * 0.12 * 2.5));
...
function summarise(items, GST_RATE) {
  ...
  return { subtotal, gst, grandTotal: subtotal + gst, totalItems: items.length };
}
...
export function calcStandardBOQ(inputs) {
  ...
  const allItems   = [];
  let   totalMat   = { cement: 0, steel: 0, sand: 0, aggregate: 0, bricks: 0, tiles: 0, paint: 0 };
  ...
  for (let i = 0; i < totalFloors; i++) {
    const { items, matQty } = buildFloorItems(plotArea, plotArea, STD, i, i === totalFloors - 1);
    allItems.push(...items);
    totalMat = mergeMatQty(totalMat, matQty);
  }

  allItems.forEach((item, idx) => { item.srNo = idx + 1; });
  ...
  return {
    ...
    materialQty: { ...Object.fromEntries(Object.entries(totalMat).map(([k, v]) => [k, r2(v)])) },
...
export function calcPremiumBOQ(inputs) {
  ...
  for (let i = 0; i < totalFloors; i++) {
    const { items, matQty } = buildFloorItems(plotArea, plotArea, PRE, i, i === totalFloors - 1);
    allItems.push(...items);
    totalMat = mergeMatQty(totalMat, matQty);
  }
...
export function calcFloorWiseBOQ(inputs) {
  const { length, breadth, floors, includeBasement, finishGrade = 'standard' } = inputs;
  ...
  fItems.forEach((item, idx) => { item.srNo = idx + 1; });
  sheets.push({
    floorLabel:  floorName(-1),
    floorIndex:  -1,
    items:       fItems,
    summary:     summarise(fItems, RATES.GST_RATE),
    materialQty: { ...Object.fromEntries(Object.entries(fMat).map(([k, v]) => [k, r2(v)])) },
  ...
  for (let i = 0; i < totalFloors; i++) {
    const { items, matQty } = buildFloorItems(
      plotArea, plotArea, RATES, i, i === totalFloors - 1,
    );
    items.forEach((item, idx) => { item.srNo = idx + 1; });
    sheets.push({
      floorLabel:  floorName(i),
      floorIndex:  i,
      items,
      summary:     summarise(items, RATES.GST_RATE),
      materialQty: { ...Object.fromEntries(Object.entries(matQty).map(([k, v]) => [k, r2(v)])) },
